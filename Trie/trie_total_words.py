...
def total_words(root):
    ...
    # Initialize variable to keep count of words
    results = 0

    # A leaf denotes the end of a word, if found add to count
    if root.is_end_word:
        results += 1

    # Recursively traverse all non-null children nodes of the current node
    for letter in root.children:
        if letter is not None:
            results += total_words(letter)
    
    return results



# Time Complexity = O(n) with n being total number of nodes
# Space Complexity = O(m) with m being total number of words
